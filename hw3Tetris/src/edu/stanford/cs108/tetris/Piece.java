...
public class Piece {
	...
	private int width;
	private int height;
	private Piece next; // "next" rotation // This is the pointer to the next piece
	...
	*/
	public Piece(TPoint[] points) { // This is a constructor
		int maximumX = 0;
		int maximumY = 0;
		
		body = new TPoint[points.length];		
		for (int c = 0; c < body.length; c++) {
			maximumX = Math.max(maximumX, points[c].x);
			maximumY = Math.max(maximumY, points[c].y);
			body[c] = new TPoint(points[c]);
		}
		...
		for (int c = 0; c < body.length; c++) {
			skirt[body[c].x] = Math.min(body[c].y, skirt[body[c].x]);
		}
	 ...
	 * (provided)
	 */
	public Piece(String points) { // This is the 2nd constructor
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] coords = new TPoint[body.length];
		int deltaY = height - 1;
		for (int c = 0; c < body.length; c++) { 
			// Use rotation matrix to transform original point to rotated point
			// R = (cos(theta) -sin(theta); sin(theta) cos(theta)), set theta = 90 degrees
			coords[c] = new TPoint(-body[c].y + deltaY, body[c].x);
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		List<TPoint> bodyCoords1 = Arrays.asList(body);
		List<TPoint> bodyCoords2 = Arrays.asList(other.body);
		boolean result = bodyCoords1.containsAll(bodyCoords2);
		return result;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while (!root.equals(nextPiece)) {
			currPiece.next = nextPiece;
			currPiece = nextPiece;
			nextPiece = currPiece.computeNextRotation();
		}
